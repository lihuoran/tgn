...
from utils.utils import EarlyStopMonitor, get_neighbor_finder, RandEdgeSampler
...
train_rand_sampler = RandEdgeSampler(train_data.src_ids, train_data.dst_ids)
val_rand_sampler = RandEdgeSampler(full_data.src_ids, full_data.dst_ids, seed=0)
nn_val_rand_sampler = RandEdgeSampler(new_node_val_data.src_ids, new_node_val_data.dst_ids, seed=1)
test_rand_sampler = RandEdgeSampler(full_data.src_ids, full_data.dst_ids, seed=2)
nn_test_rand_sampler = RandEdgeSampler(new_node_test_data.src_ids, new_node_test_data.dst_ids, seed=3)
...
mean_time_shift_src, std_time_shift_src, mean_time_shift_dst, std_time_shift_dst = \
    compute_time_statistics(full_data.src_ids, full_data.dst_ids, full_data.timestamps)

for i in range(args.n_runs):
    results_path = f"results/{args.prefix}_{i}.pkl" if i > 0 else f"results/{args.prefix}.pkl"
    ...
    num_instance = len(train_data.src_ids)
    ...
    for epoch in range(args.n_epoch):
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                src_batch = train_data.src_ids[start_idx:end_idx]
                dst_batch = train_data.dst_ids[start_idx:end_idx]
                edge_idxes_batch = train_data.edge_idxes[start_idx:end_idx]
        ...
        logger.info(f'val auc: {val_auc}, new node val auc: {nn_val_auc}')
        logger.info(f'val ap: {val_ap}, new node val ap: {nn_val_ap}')
